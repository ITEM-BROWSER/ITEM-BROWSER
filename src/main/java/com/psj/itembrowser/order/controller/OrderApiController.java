...
public class OrderApiController {
	...
	public ResponseEntity<PageInfo<OrderResponseDTO>> getOrders(
		...
		if (member.hasRole(Member.Role.ROLE_ADMIN)) {
			orderResponseDTOPageInfo = orderService.getOrdersWithPaginationAndNoCondition(orderPageRequestDTO);
		} else if (member.hasRole(Member.Role.ROLE_CUSTOMER)) {
			orderResponseDTOPageInfo = orderService.getOrdersWithPaginationAndNotDeleted(orderPageRequestDTO);
		}
